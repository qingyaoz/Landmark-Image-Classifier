...
class ImageStandardizer(object):
    ...
    def fit(self, X):
        ...
        # 0 (batch), 1 (height), and 2 (width) axes.
...
class LandmarksDataset(Dataset):
    ...
    def __init__(self, partition, task="target", augment=False):
        ...
        `task` could be "target" or "source" for different purposes.
    ...
    def _load_data(self):
        ...
        for i, row in df.iterrows():
            ...
            image = imread(os.path.join(path, row["filename"]))#.astype(np.float64) # 确保图片输入方式？
            X.append(image)
